...
const getUserData = (data, checks) => {
  ...
  if (checks.includes('isNotVerified') && user.is_verified) {
  ...
  if (checks.includes('isNotVerifiedOrHasVerifyChanges') &&
    user.is_verified && !Object.keys(user.verify_changes || {}).length
  ) {
  ...
  if (checks.includes('resetNotExpired') && user.reset_expires < Date.now()) {
...
const sanitizeUserForClient = user => {
  ...
  delete user1.verify_expires;
  delete user1.verify_token;
  delete user1.verify_short_token;
  delete user1.verify_changes;
  delete user1.reset_expires;
  delete user1.reset_token;
  delete user1.reset_short_token;
