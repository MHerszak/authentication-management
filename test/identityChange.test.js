...
const usersDb = [
  { _id: 'a', email: 'a', plainPassword: 'aa', is_verified: false },
  { _id: 'b', email: 'b', plainPassword: 'bb', is_verified: true },
];
        ...
        it('updates verified user', function (done) {
          ...
          authManagement.create({ action: 'identityChange', value: {
            ...
            .then(user1 => {
              ...
              assert.strictEqual(user1.is_verified, true, 'is_verified not true');
              assert.equal(dbi.email, user.email);
              assert.deepEqual(dbi.verify_changes, { email });
              ...
              assert.deepEqual(
                ...
                [
                  ...
                  Object.assign({},
                    ...
                    extractProps(
                      db[i], 'verify_expires', 'verify_token', 'verify_short_token', 'verify_changes'
                    )
                  ),
                  {}
                ],
              ...
              assert.strictEqual(dbi.is_verified, true, 'is_verified not false');
              assert.isString(dbi.verify_token, 'verify_token not String');
              assert.equal(dbi.verify_token.length, 30, 'verify token wrong length');
              assert.equal(dbi.verify_short_token.length, 6, 'verify short token wrong length');
              assert.match(dbi.verify_short_token, /^[0-9]+$/);
